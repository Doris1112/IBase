 ...
 */
public class ISwipeHelper {

    private final int START_SLIDE_X = 30;

    private static ISwipeHelper sSwipeHelper;
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (value instanceof Float && curContentView != null) {
                    ...
                    if (aFloat >= mScreenWidth) {
                        IActivityLifeHelper.instance().getCurrentActivity().finish();
                    }
                }
            }
        });
    ...
    public static ISwipeHelper instance() {
        if (sSwipeHelper == null) {
            synchronized (IWindowHelper.class) {
                if (sSwipeHelper == null) {
                    sSwipeHelper = new ISwipeHelper();
                }
            }
        }
        return sSwipeHelper;
    ...
    private View getCurContentView() {
        Activity curActivity = IActivityLifeHelper.instance().getCurrentActivity();
        if (curActivity != null) {
            return IWindowHelper.instance().getContentView(curActivity);
        }
        return null;
    }

    private View getPreContentView() {
        Activity preActivity = IActivityLifeHelper.instance().getPreviewActivity();
        if (preActivity != null) {
            return IWindowHelper.instance().getContentView(preActivity);
        }
        return null;
    }
    ...
    public boolean processTouchEvent(MotionEvent ev) {
        ...
        Activity curActivity = IActivityLifeHelper.instance().getCurrentActivity();
        if (mScreenWidth <= 0) {
            mScreenWidth = IWindowHelper.instance().getScreenWidth(curActivity);
